...
// 31-Aug-2023 insane:
//   modifications for symbos regarding memory paging and interrupts
//   fix LD A,R -> old code cleared I register
...
static INLINE unsigned char z80_read(FASTWORK address) {
  return(z80_bank[address>>BANK_SHIFT][address&BANK_MASK]);
};

static INLINE void z80_write(FASTWORK address, FASTWORK value) {
  z80_bank[address>>BANK_SHIFT][address&BANK_MASK]=value;
};

static INLINE unsigned int z80_readw(FASTWORK address) {
  return((z80_bank[address>>BANK_SHIFT][address&BANK_MASK])+
         (z80_bank[(address+1)>>BANK_SHIFT][(address+1)&BANK_MASK]<<8));
};

static INLINE void z80_writew(FASTWORK address, FASTWORK value) {
  z80_bank[address>>BANK_SHIFT][address&BANK_MASK]=value;
...
#define mm_GetBYTE(a)   z80_read(--a)
#define PutBYTE(a, v)   z80_write(a,v)
#define PutBYTE_pp(a,v) z80_write(a++,v)
#define PutBYTE_mm(a,v) z80_write(a--,v)
#define mm_PutBYTE(a,v) z80_write(--a,v)
#define GetWORD(a)      z80_readw(a)
#define PutWORD(a, v)   z80_writew(a,v)
...
/* load Z80 registers into host registers */
#define LOAD_STATE()             \
    WORD PC = reg_pc;              \
    WORD AF = reg_af[reg_af_sel];            \
    WORD BC = regs[regs_sel].bc;         \
    WORD DE = regs[regs_sel].de;         \
    WORD HL = regs[regs_sel].hl;         \
    WORD IX = reg_ix;              \
    WORD IY = reg_iy;              \
    WORD SP = reg_sp
...
#define CHECK_IRQ() \
    if((implant_IRQ)&&(reg_IFF&1)) { \
      implant_IRQ=reg_IFF=0; \
      PUSH(PC); \
      PC=implant_PC; \
    }
...
int simz80_run(void) {
  ...
  while (z80_running) {
    switch(GetBYTE_pp(PC)) {
      ...
      case 0x08:      /* EX AF,AF' */
        reg_af[reg_af_sel] = AF;
        reg_af_sel = 1 - reg_af_sel;
        AF = reg_af[reg_af_sel];
        break;
      case 0x09:      /* ADD HL,BC */
        HL &= 0xffff;
        BC &= 0xffff;
      ...
      case 0xED:      /* ED prefix */
        switch (op = GetBYTE_pp(PC)) {
          ...
          case 0x5F:      /* LD A,R */
            ...
            AF = (AF & 0x29) | ((reg_ir & 255) << 8) | (reg_ir & 0x80) | (((reg_ir & 255) == 0) << 6) | ((reg_IFF & 2) << 1);
            break;
          case 0x60:      /* IN H,(C) */
            temp = Input(BC);
            Sethreg(HL, temp);
...
void simz80_getregs(z80_regs *getregs) {
  getregs->af=(reg_af[reg_af_sel]&0xffff)|((reg_af[reg_af_sel^1]&0xffff)<<16);
  getregs->bc=(regs[regs_sel].bc&0xffff)|((regs[regs_sel^1].bc&0xffff)<<16);
  getregs->de=(regs[regs_sel].de&0xffff)|((regs[regs_sel^1].de&0xffff)<<16);
  getregs->hl=(regs[regs_sel].hl&0xffff)|((regs[regs_sel^1].hl&0xffff)<<16);
  getregs->ir=(reg_ir)&0xffff;
  getregs->ix=(reg_ix)&0xffff;
  getregs->iy=(reg_iy)&0xffff;
  getregs->sp=(reg_sp)&0xffff;
  getregs->pc=(reg_pc)&0xffff;
  getregs->im=(reg_im)&0xffff;
  getregs->IFF=(reg_IFF)&0xffff;
};

void simz80_setregs(z80_regs *setregs) {
  reg_af_sel=regs_sel=0;
  reg_af[0]=(setregs->af)&0xffff;
  reg_af[1]=(setregs->af>>16)&0xffff;
  regs[0].bc=(setregs->bc)&0xffff;
  regs[0].de=(setregs->de)&0xffff;
  regs[0].hl=(setregs->hl)&0xffff;
  regs[1].bc=(setregs->bc>>16)&0xffff;
  regs[1].de=(setregs->de>>16)&0xffff;
  regs[1].hl=(setregs->hl>>16)&0xffff;
  reg_ir=(setregs->ir)&0xffff;
  reg_ix=(setregs->ix)&0xffff;
  reg_iy=(setregs->iy)&0xffff;
  reg_sp=(setregs->sp)&0xffff;
  reg_pc=(setregs->pc)&0xffff;
  reg_im=(setregs->im)&0xffff;
  reg_IFF=(setregs->IFF)&0xffff;
};

void simz80_setbank(int reg, int dat) {
  z80_bank[reg&3]=&z80_mem[(dat&0xff)<<14];
  z80_bankreg[reg&3]=dat;
};
